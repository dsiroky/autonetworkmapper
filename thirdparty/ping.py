...
class PingQuery(asyncore.dispatcher):
    def __init__(self, host, p_id, timeout=0.5, ignore_errors=False, ttl=100):
        """
...
def multi_ping_query(hosts, timeout=1, step=512, ignore_errors=False, ttl=100):
    """
    ...
    while host_list:
        ...
        for ip in host_list[:step]: # select supports only a max of 512
            id += 1
            sock_list.append(PingQuery(ip, id, timeout, ignore_errors, ttl))
            host_list.remove(ip)
